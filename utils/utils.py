...
def plot_training_trace(loss_list, acc_list):

    loss_list = [l.item() for l in loss_list]
    # acc_list = [l.item() for l in acc_list]
    plt.plot(np.array(acc_list))
    plt.xlabel("epoch")
    plt.ylabel("Accuracy")
    plt.title("Binary classification accuracy")
    plt.savefig("/home/wuhaoran/sscp2/fig/train_accuracy.png")
    plt.close()

    plt.plot(np.array(loss_list))
    plt.xlabel("epoch")
    plt.ylabel("loss")
    plt.title("SSL loss function")
    plt.savefig("/home/wuhaoran/sscp2/fig/train_loss.png")
    plt.close()
