...
class EvolutionaryScaleModeling(nn.Module, core.Configurable):
    ...
    def __init__(self, path, model="esm2_t12_35M_UR50D", readout="mean"):
    ...
    def load_weight(self, path, model):
        ...
        if model != "ESM-1v":
            regression_file = path + model + "-contact-regression.pt"
            regression_data = torch.load(regression_file, map_location="cpu")
        else:
            regression_data = None
        # model_name = os.path.basename(self.url[model])
        # print(help(esm.pretrained.load_model_and_alphabet_core()))
        # print(dir(esm))
        return esm.load_model_and_alphabet_core(model, model_data, regression_data)
        # return load_model_and_alphabet_core(model, model_data, regression_data)
...
if __name__ == '__main__':
    ...
    path = '/tmp/esm2_t12/'
    print(esm.MSATransformer)
    aa = EvolutionaryScaleModeling(path)
    print(aa)