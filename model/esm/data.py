...
from model.esm.constants import proteinseq_toks
...
class FastaBatchedDataset(object):
    ...
    def from_file(cls, fasta_file):
        ...
        assert len(set(sequence_labels)) == len(
            sequence_labels), "Found duplicate sequence labels"
...
class Alphabet(object):
    ...
    def from_architecture(cls, name: str) -> "Alphabet":
        ...
        return cls(standard_toks, prepend_toks, append_toks, prepend_bos,
                   append_eos, use_msa)
...
class BatchConverter(object):
    ...
    def __call__(self, raw_batch: Sequence[Tuple[str, str]]):
        ...
        seq_encoded_list = [
            self.alphabet.encode(seq_str) for seq_str in seq_str_list
        ]
        max_len = max(len(seq_encoded) for seq_encoded in seq_encoded_list)
        tokens = torch.empty(
            (
                batch_size,
                max_len + int(self.alphabet.prepend_bos) +
        ...
        for i, (label, seq_str, seq_encoded) in enumerate(
            ...
            tokens[i,
                   ...
                   int(self.alphabet.prepend_bos), ] = seq
            if self.alphabet.append_eos:
                tokens[i, len(seq_encoded) +
                       int(self.alphabet.prepend_bos)] = self.alphabet.eos_idx
    ...
    def __call__(self, inputs: Union[Sequence[RawMSA], RawMSA]):
        ...
        tokens = torch.empty(
            (
                batch_size,
                max_alignments,
                max_seqlen + int(self.alphabet.prepend_bos) +
                int(self.alphabet.append_eos),
            ),
            dtype=torch.int64,
        )
...
class ESMStructuralSplitDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.split_file = os.path.join(self.base_path, "splits", split_level,
                                       cv_partition, f"{split}.txt")
